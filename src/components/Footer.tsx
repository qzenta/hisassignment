      ...
      <div className="max-w-6xl mx-auto px-6 py-14 grid grid-cols-1 md:grid-cols-3 gap-10">
        ...
        <div>
          ...
          <ul className="flex flex-col gap-2">
            {services.map((s) => (
              <li key={s}>
                <Link href="/services" className="text-sm text-white hover:text-[#00BFA5] transition-colors">
                  {s}
                </Link>
              </li>
            ))}
        ...
        <div>
          ...
          <ul className="flex flex-col gap-3 text-sm text-[#CBD5E1]">
            <li className="flex gap-2">
              ...
              396 Elgin Street, Ferndale, Randburg, 2194
            </li>
            <li className="flex gap-2">
            ...
            <li className="flex gap-2">
              ...
              <a href="mailto:payroll@hisassignment.co.za" className="hover:text-[#00BFA5] transition-colors">payroll@hisassignment.co.za</a>
            </li>
          </ul>
          <div className="mt-6 flex gap-3 flex-wrap">
            {["navLinks"].map(() =>
              navLinks.map((l) => (
                <Link key={l.href} href={l.href} className="text-xs text-[#94A3B8] hover:text-white transition-colors">
                  {l.label}
                </Link>
              ))
            )}
      ...
      <div className="border-t border-[#2D4F8A] py-5">
        <div className="max-w-6xl mx-auto px-6 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-[#64748B]">
          ...
          <div className="flex gap-4">
            <span className="text-[#00BFA5]">SARS Registered</span>
            <span>·</span>
            <span className="text-[#00BFA5]">PoPIA Compliant</span>
          </div>
        </div>
      </div>
    </footer>
